...
def get_k_means_plus_plus_center_indices(n, n_cluster, x, generator=np.random):
    ...
    while cluster_count < n_cluster:
        ...
        for point in range(n):
            #if point not in cinds:                
            probs.append(np.min(np.linalg.norm(x[point] - np.array(centers), axis=1) ** 2, axis=0))                                
                
        probs = np.array(probs) / np.sum(probs)
        r = generator.rand()
        i = 0
        while i < n - 1:
            if r < np.sum(probs[:i+1]):
                centers.append(x[i])
                cinds.append(i)                
                cluster_count += 1
                break
...
class KMeans():
    ...
    def fit(self, x, centroid_func=get_lloyd_k_means):
        ...
        sample_ass = np.zeros((N,))
        K_obj = np.inf        
        clusters = x[self.centers]        
        for i in range(self.max_iter):
            current_K_obj = 0
            for point in range(N):
                cluster_ind = np.argmin(np.linalg.norm(x[point] - clusters, axis=1), axis = 0)
                current_K_obj += np.linalg.norm(x[point] - clusters[cluster_ind]) ** 2
                sample_ass[point] = cluster_ind
                #sample_ass[point, 1] += 1
                
            if np.abs(K_obj - current_K_obj / N) < self.e:
                break

            K_obj = current_K_obj / N

            # cluster assignment
            for cluster in range(self.n_cluster):
                if np.sum(sample_ass == cluster):
                    clusters[cluster] = np.mean(x[sample_ass == cluster], axis=0)            
                
        return (clusters, sample_ass, i)
...
class KMeansClassifier():
    ...
    def fit(self, x, y, centroid_func=get_lloyd_k_means):
        ...
        centroids, samples, _ = kmeans.fit(x, centroid_func)
        centroid_labels = []
        for i in range(self.n_cluster):
            uniques = np.unique(y[samples == i], return_counts=True)
            centroid_labels.append(uniques[0][np.argmax(uniques[1])])
    ...
    def predict(self, x):
        ...
        labels = []
        for i in range(N):
            labels.append(self.centroid_labels[np.argmin(np.linalg.norm(x[i] - self.centroids, axis=1), axis=0)])
...
def transform_image(image, code_vectors):
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            new_image[i, j] = code_vectors[np.argmin(np.linalg.norm(image[i, j] - code_vectors, axis=1), axis=0)]
